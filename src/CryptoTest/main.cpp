...
void testHashFunction(T hashFunction, std::string expectedOutput,
  ...
  if (height == -1) {
    ...
    std::cout << hashFunctionName << " (" << height << "): " << hash
              << std::endl;
  }
...
void benchmarkUnderivePublicKey() {
  ...
  /* Need to use microseconds here then divide by 1000 - otherwise we'll just
   * get '0' */
  ...
  std::cout << "Time to perform underivePublicKey: "
            << timePerDerivation / 1000.0 << " ms" << std::endl;
}
...
void benchmarkGenerateKeyDerivation() {
  ...
  std::cout << "Time to perform generateKeyDerivation: "
            << timePerDerivation / 1000.0 << " ms" << std::endl;
}
...
int main(int argc, char **argv) {
  ...
  options.add_options("Performance Testing")(
      "b,benchmark", "Run quick performance benchmark", cxxopts::value<bool>(o_benchmark)
                                                 ->default_value("false")
                                                 ->implicit_value("true"))(
      "i,iterations", "The number of iterations for the benchmark test. Minimum of 1,000 iterations required.", cxxopts::value<int>(o_iterations)
          ->default_value(std::to_string(PERFORMANCE_ITERATIONS)), "#");

  try {
    auto result = options.parse(argc, argv);
  } catch (const cxxopts::OptionException &e) {
    std::cout << "Error: Unable to parse command line argument options: "
              << e.what() << std::endl
              << std::endl;
  ...
  if (o_iterations < 1000 && o_benchmark) {
    std::cout << std::endl
              << "Error: The number of --iterations should be at least 1,000 "
  ...
  try {
    ...
    if (o_benchmark) {
      std::cout << "\nPerformance Tests: Please wait, this may take a while "
