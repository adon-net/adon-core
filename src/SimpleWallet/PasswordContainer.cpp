...
PasswordContainer::PasswordContainer(PasswordContainer&& rhs) : m_empty(std::move(rhs.m_empty)), m_password(std::move(rhs.m_password)) {
}
...
bool PasswordContainer::read_password(bool verify) {
    ...
    bool r; //, present = false, cpresent = false;
    if (is_cin_tty()) {
        std::cout << "PASSWORD         # ";
        if (verify) {
