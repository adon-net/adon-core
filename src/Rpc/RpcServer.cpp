...
bool RpcServer::processJsonRpcRequest(const HttpRequest& request, HttpResponse& response) {
  ...
  try {
    ...
    static std::unordered_map<std::string, RpcServer::RpcHandler<JsonMemberMethod>> jsonRpcHandlers = {
        ...
        {"transactions_by_payment_id", { makeMemberMethod(&RpcServer::on_transactions_by_payment_id), false } },
...
bool RpcServer::mn_check_incoming_tx(const BinaryArray& tx_blob) {
	...
	if (amount != 0) {
		logger(INFO) << "Masternode received relayed transaction fee: " << m_core.currency().formatAmount(amount) << " " << CRYPTONOTE_TICKER;
		return true;
	}
	return false;
}
...
bool RpcServer::on_send_raw_tx(const COMMAND_RPC_SEND_RAW_TX::request& req, COMMAND_RPC_SEND_RAW_TX::response& res) {
  ...
  if (!m_fee_address.empty() && m_view_key != NULL_SECRET_KEY) {
    if (!mn_check_incoming_tx(tx_blob)) {
...
bool RpcServer::f_on_blocks_list_json(const F_COMMAND_RPC_GET_BLOCKS_LIST::request& req, F_COMMAND_RPC_GET_BLOCKS_LIST::response& res) {
    ...
    uint32_t print_blocks_count = 100;
...
bool RpcServer::f_on_block_json(const F_COMMAND_RPC_GET_BLOCK_DETAILS::request& req, F_COMMAND_RPC_GET_BLOCK_DETAILS::response& res) {
    ...
    res.block.full_deposit_amount = m_core.fullDepositAmount();
    res.block.full_deposit_interest = m_core.fullDepositInterest();
...
bool RpcServer::on_transactions_by_payment_id(const COMMAND_RPC_GET_TRANSACTIONS_BY_PAYMENT_ID::request& req, COMMAND_RPC_GET_TRANSACTIONS_BY_PAYMENT_ID::response& res) {
...
bool RpcServer::onGetTransactionsDetailsByHashes(const COMMAND_RPC_GET_TRANSACTIONS_DETAILS_BY_HASHES::request& req, COMMAND_RPC_GET_TRANSACTIONS_DETAILS_BY_HASHES::response& rsp) {
  try {
    std::vector<TransactionDetailsEx> transactionsDetails;
    ...
    if (!txs.empty()) {
      for (const Transaction& tx: txs) {
        TransactionDetailsEx txDetails;
...
bool RpcServer::onGetTransactionDetailsByHash(const COMMAND_RPC_GET_TRANSACTION_DETAILS_BY_HASH::request& req, COMMAND_RPC_GET_TRANSACTION_DETAILS_BY_HASH::response& rsp) {
  ...
  TransactionDetailsEx transactionsDetails;
...
bool RpcServer::on_check_tx_with_view_key(const COMMAND_RPC_CHECK_TX_WITH_PRIVATE_VIEW_KEY::request& req, COMMAND_RPC_CHECK_TX_WITH_PRIVATE_VIEW_KEY::response& res) {
    // parse txid
    Crypto::Hash txid;
    if (!parse_hash256(req.txid, txid)) {
        throw JsonRpc::JsonRpcError{CORE_RPC_ERROR_CODE_WRONG_PARAM, "Failed to parse txid"};
...
bool RpcServer::on_check_tx_proof(const COMMAND_RPC_CHECK_TX_PROOF::request& req, COMMAND_RPC_CHECK_TX_PROOF::response& res) {
    // parse txid
    Crypto::Hash txid;
...
bool RpcServer::on_check_reserve_proof(const COMMAND_RPC_CHECK_RESERVE_PROOF::request& req, COMMAND_RPC_CHECK_RESERVE_PROOF::response& res) {
    ...
    for (size_t i = 0; i < proofs.size(); ++i) {
        ...
        if (!Crypto::check_ring_signature(prefix_hash, proof.key_image, &pubs[0], 1, &proof.key_image_sig, true)) {
