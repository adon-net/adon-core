...
uint64_t Blockchain::getMinimalFee(int32_t height) {
	...
	for (; offset < static_cast<size_t>( height ); offset++) {
		timestamps.push_back(m_blocks[offset].bl.timestamp);
...
bool Blockchain::check_tx_input(const KeyInput& txin, const Crypto::Hash& tx_prefix_hash, const std::vector<Crypto::Signature>& sig, uint32_t* pmax_related_block_height) {
  ...
  bool check_tx_ring_signature = Crypto::check_ring_signature(tx_prefix_hash, txin.keyImage, output_keys, sig.data(), true);
...
bool Blockchain::update_next_comulative_size_limit() {
  ...
  //size_t blockGrantedFullRewardZone = getBlockMajorVersionForHeight(getCurrentBlockchainHeight()) < parameters::UPGRADE_HEIGHT_V4 ? parameters::CRYPTONOTE_BLOCK_GRANTED_FULL_REWARD_ZONE_V1 : m_currency.blockGrantedFullRewardZone();
...
bool Blockchain::pushBlock(const Block& blockData, const std::vector<Transaction>& transactions, block_verification_context& bvc) {
  ...
  if (!(currentDifficulty)) {
    logger(ERROR, BRIGHT_RED) << "!!!!!!!!! difficulty overhead !!!!!!!!!" << currentDifficulty;
    return false;
  }
  ...
  logger(DEBUGGING) << "+++++ BLOCK SUCCESSFULLY ADDED" 
                    << ENDL << "ID          " << blockHash
                    << ENDL << "PoW         " << proof_of_work
