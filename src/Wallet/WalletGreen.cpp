...
size_t WalletGreen::transfer(const TransactionParameters& transactionParameters){
  ...
  return WalletGreen::transfer(transactionParameters, txSecretKey);
}
...
void WalletGreen::prepareTransaction(std::vector<WalletOuts>&& wallets,
  ...
  typedef CryptoNote::RandomOuts outs_for_amount;
...
void WalletGreen::checkIfEnoughMixins(std::vector<CryptoNote::RandomOuts>& mixinResult, uint64_t mixIn) const {
  assert(mixIn != 0);

  auto notEnoughIt = std::find_if(mixinResult.begin(), mixinResult.end(),
    [mixIn] (const CryptoNote::RandomOuts& ofa) { return ofa.outs.size() < mixIn; } );
...
uint64_t WalletGreen::pushDonationTransferIfPossible(const DonationSettings& donation, uint64_t freeAmount,
  uint64_t dustThreshold, std::vector<WalletTransfer>& destinations) const {  uint64_t donationAmount = 0;
...
void WalletGreen::prepareInputs(
  ...
  typedef CryptoNote::OutputEntry out_entry;
...
std::vector<WalletGreen::OutputToTransfer> WalletGreen::pickRandomFusionInputs(const std::vector<std::string>& addresses,
  ...
  std::shuffle(bucketNumbers.begin(), bucketNumbers.end(), std::default_random_engine{Random::randomValue<std::default_random_engine::result_type>()});
