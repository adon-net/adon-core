...
struct IWalletBaseStub : public CryptoNote::IWallet, public CryptoNote::IFusionManager {
  ...
  virtual Crypto::SecretKey getTransactionSecretKey(size_t transactionIndex) const override { return NULL_SECRET_KEY; }
...
TEST_F(WalletServiceTest_sendTransaction, passesCorrectParameters) {
  ...
  std::string hash;
  std::string txKey;
  auto ec = service->sendTransaction(request, hash, txKey);
