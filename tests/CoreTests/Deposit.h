...
struct EmissionTest : public DepositTestsBase {
  ...
  bool check_block_verification_context(const CryptoNote::block_verification_context& bvc, std::size_t eventIdx,
    ...
    return emission_after == emission_before + CryptoNote::parameters::GENESIS_BLOCK_REWARD + m_currency.calculateInterest(m_currency.depositMinAmount(), m_currency.depositMinTerm());
  }
...
struct EmissionTestRestore : public EmissionTest {
  bool check_block_verification_context(const CryptoNote::block_verification_context& bvc, std::size_t eventIdx,
    ...
    return emission_after == emission_before + CryptoNote::parameters::GENESIS_BLOCK_REWARD * 3 - m_currency.calculateInterest(m_currency.depositMinAmount(), m_currency.depositMinTerm());
  }
};
